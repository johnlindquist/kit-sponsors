// Menu: egghead.io
// Description: Browse egghead courses and lessons
// Author: egghead.io
// Twitter: @eggheadio
    ...
    if (input) {
      ...
      return (hits || []).map(hit => {
        return {
          ...
          preview: async () => {
            // We _could_ query the egghead api here, but I think we have enough info already? 🤔
            ...
            if (lessons?.length) {
              setFlags({
                instructor: {
                  ...
                  description:
                    "Open the instructor profile in your browser",
                  shortcut: "cmd+i",
                ...
                twitter: {
                  ...
                  description:
                    "Open the instructor twitter account",
                  shortcut: "cmd+t",
                ...
                website: {
                  ...
                  description:
                    "Open the instructor website",
                  shortcut: "cmd+w",
                },
                ...lessons?.map((lesson, i) => ({
                  name: `${i + 1}: ${lesson.title}`,
                  description: convertTimeWithTitles(
                    lesson.duration
                  ),
                  shortcut: `cmd+${i + 1}`,
                })),
              ...
              <div class="uppercase text-xs pb-2 tracking-wide opacity-60 px-5 pt-5">Lessons</div>
                  ${lessons
                    ?.map((lesson, i) => {
                      ...
                      <span class="opacity-50 pr-2 text-xs">${
                        i + 1
                      }</span>
                      ${lesson.title}
                      </a>
...
// - Script "frontmatter" parser takes all instances of `// shortcut:
